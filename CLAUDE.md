...
Site is served from **GitHub Pages at the root** of the custom domain https://ekylibre.org/ (with https://ekylibre.github.io/ redirecting to it). The domain is pinned via `content/CNAME` (passed through to `public/CNAME` by Nanoc's catch-all rule). All asset URLs in `layouts/start.html`, `layouts/docs.html`, and `content/index.html` are root-relative (`/…`) — do **not** reintroduce a sub-path prefix.
...
Deploy: push to `master`. `.github/workflows/pages.yml` builds and publishes to GitHub Pages — no manual deploy command. The Pages site is configured with custom domain `ekylibre.org` (set in repo Settings → Pages and pinned via `content/CNAME`); DNS for `ekylibre.org` points to GitHub Pages.
